...
class SwitchServerRequires(RelationBase):
    scope = scopes.UNIT

    @hook('{requires:switch-server}-relation-{joined,changed}')
    def changed(self):
        conv = self.conversation()
        if conv.get_remote('anotherConfig'):
            # this unit's conversation has a port, so
            # it is part of the set of available units
            conv.set_state('{switch-server}.available')

    @hook('{requires:switch-server}-relation-{departed}')
    def departed(self):
        conv = self.conversation()
        conv.remove_state('{switch-server}.available')

    def services(self):
        """
        Returns a list of available switch-server services and their associated hosts
        and configs.
