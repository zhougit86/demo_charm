...
class RackPduRequires(RelationBase):
    scope = scopes.UNIT

    @hook('{requires:rack-pdu}-relation-{joined,changed}')
    def changed(self):
        conv = self.conversation()
        if conv.get_remote('anotherConfig'):
            # this unit's conversation has a port, so
            # it is part of the set of available units
            conv.set_state('{rack-pdu}.available')

    @hook('{requires:rack-pdu}-relation-{departed}')
    def departed(self):
        conv = self.conversation()
        conv.remove_state('{rack-pdu}.available')

    def services(self):
        """
        Returns a list of available rack-pdu services and their associated hosts
        and configs.
