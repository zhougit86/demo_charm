...
class ServerRaidRequires(RelationBase):
    scope = scopes.UNIT

    @hook('{requires:server-raid}-relation-{joined,changed}')
    def changed(self):
        conv = self.conversation()
        if conv.get_remote('anotherConfig'):
            # this unit's conversation has a port, so
            # it is part of the set of available units
            conv.set_state('{server-raid}.available')

    @hook('{requires:server-raid}-relation-{departed}')
    def departed(self):
        conv = self.conversation()
        conv.remove_state('{server-raid}.available')

    def services(self):
        """
        Returns a list of available server-raid services and their associated hosts
        and configs.
